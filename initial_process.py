...
def initial_process(basedir, mopex, channel='ch1', nprocesses=8):
    ...
    def run_dir(dr):
        ...
        with open(os.path.join(wd, 'run.sh'), 'w') as f:
            ...
            f.write(f'overlap.pl -n overlap_{bandname}.nl -I images.list -S sigma.list -d mask.list > log1.txt \n')
            f.write(f'mosaic.pl -n mosaic_{bandname}.nl -I images.list -S sigma.list -d mask.list > log2.txt \n')
