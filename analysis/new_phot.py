...
def get_data(settings, all_data, use_masked_dqs=False):
    ...
    for i in range(settings["n_img"]):
        ...
        if "median_weighting" in settings.keys() and settings["median_weighting"]==1:
            invvars = (ones(data[1].shape)*median(data[1])*settings["errscale"][i])**-2. * (DQ == 0)
        else:
