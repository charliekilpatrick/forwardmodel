...
#uri='https://irsa.ipac.caltech.edu/cgi-bin/Atlas/nph-atlas'
...
def download_from_coord(coord, outdir='./', filetypes=['cbcd','cbunc','bimsk'],
    ...
    imask = np.array(['IRAC' in row['modedisplayname'] for row in table])

    if channel=='ch1':
        fmask = table['wavelength']=='IRAC 3.6um'
    elif channel=='ch2':
        fmask = table['wavelength']=='IRAC 4.5um'
    elif channel=='ch3':
        fmask = table['wavelength']=='IRAC 5.4um'
    elif channel=='ch4':
        fmask = table['wavelength']=='IRAC 8.0um'
    else:
        raise Exception(f'Bad wavelength {channel}')

    table = table[imask & fmask]
    n = len(table)
    ...
    for row in table:
        for filetype in filetypes:
            outfiles.append(get_file(row, outrootdir=outdir, filetype=filetype))
